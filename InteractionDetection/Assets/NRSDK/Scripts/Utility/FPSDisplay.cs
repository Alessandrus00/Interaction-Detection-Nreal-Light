    ...
    public class FPSDisplay : MonoBehaviour
    {
        ...
        void Update()
        {
            /*highestFPSLabel.text =
                $"Highest:  {stringsFrom00To99[Mathf.Clamp(fpsCounter.HighestFPS, 0, 99)]}";
            averageFPSLabel.text =
                $"Average:  {stringsFrom00To99[Mathf.Clamp(fpsCounter.AverageFPS, 0, 99)]}";
            lowestFPSLabel.text =
                $"Lowest:   {stringsFrom00To99[Mathf.Clamp(fpsCounter.LowestFPS, 0, 99)]}";*/
            
            averageFPSLabel.text = 
                $"Average: {(int)(Time.frameCount / Time.time)}";
        }
    }
}